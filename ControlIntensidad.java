...
public class ControlIntensidad extends JFrame {
    ...
    private void initializeCamera(int cameraIndex) {
        // Intentar autodetectar índice de cámara disponible (0..5) y probar backends comunes
        camera = null;
        int[] backends = new int[]{Videoio.CAP_DSHOW, Videoio.CAP_MSMF, Videoio.CAP_ANY};
        for (int backend : backends) {
            for (int i = 0; i < 6; i++) {
                VideoCapture cap = new VideoCapture(i, backend);
                if (cap.isOpened()) {
                    camera = cap;
                    System.out.println("Cámara abierta en índice: " + i + " con backend: " + backend);
                    break;
                } else {
                    cap.release();
                }
        ...
        if (camera == null || !camera.isOpened()) {
            JOptionPane.showMessageDialog(this,
                    "❌ No se detecta la cámara o está en uso. Cierra otras apps que la usen o conecta la cámara.",
                    "Error de Cámara",
